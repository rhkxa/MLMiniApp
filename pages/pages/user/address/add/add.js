...
exports.default = Page({
    ...
    onLoad: function onLoad(e) {
        ...
        if (ctid) {
            ...
            _server2.default.get(_urls2.default.links[0].addsdetail, { token: token, id: ctid }).then(function (res) {
                if (res.code == 0) {
                    wx.hideLoading();
                    that.setData({
    ...
    bindSave: function bindSave(e) {
        ...
        if (apiAddid == 0) {
            ;
            var addresslinks = _urls2.default.links[0].addsadd;
        } else {
            var addresslinks = _urls2.default.links[0].addupdate;
        }
        _server2.default.get(addresslinks, {
            token: token,
            id: apiAddid,
            linkMan: linkMan,
            mobile: mobile,
    ...
    deleteAddress: function deleteAddress(e) {
        ...
        wx.showConfirm({
            ...
            success: function success(res) {
                if (res.confirm) {
                    _server2.default.get(_urls2.default.links[0].adddelete, { token: token, id: id }).then(function (res) {
                        if (res.code == 0) {
                            wx.navigateBack({});
                        }
                    });
